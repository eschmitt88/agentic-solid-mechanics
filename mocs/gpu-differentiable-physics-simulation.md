...
- **Quadrants** *(added 2026-06-30, user-surfaced)* — Genesis Embodied AI's standalone GPU kernel compiler (almost certainly the Genesis engine's backend, post-Taichi-fork). Multi-backend (CUDA/Vulkan/Metal/ROCm/CPU) with reverse-mode AD; standout claim is **autodiff through dynamic loops + runtime allocation** — the exact thing Warp's tape struggles with. But tiny community (~158 stars), no shipped physics/FEM layer, and Genesis's overstated-marketing history warrants verifying the "gradient of any kernel" claim. Not vendor-aligned for Blackwell the way Warp is. Tracked, not adopted. See [[quadrants]].
